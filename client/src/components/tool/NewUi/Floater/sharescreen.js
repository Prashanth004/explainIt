...
import {getRecieverData} from '../../../../actions/callAction'
...
setNoOfMinutes(minutes){
   this.setState({
    ...
    receiveMessage() {
        var source = "[WINDOW]"
        var origin = config.react_url
        if (this.props.extSource !== null) {
            window.postMessage('audio-plus-tab', origin);
        }
    }
    expireTimer(){
        var peer = this.state.peer;
    ...
    componentDidMount() {
        ...
        socket.on(config.CALL_ACK_MESSAGE, data => {
            ...
            this.props.getRecieverData(data.recieverProfilePic,
                data.recieverProfileName,
                data.recieverUserId
            )
            self.setState({
    ...
    render() {
        ...
        return (this.state.isInstalled) ? (
            <div>
                {/* {backArrow} */}
               {/* { closeBtn} */}
                <div className="mainScreenShareContainer">
                    {linkElement}
                    {shareTimeElements}
                    {postShareElements}
...
ScreenRecorder.PropType = {
    ...
    getRecieverData:PropType.func.isRequired
}
